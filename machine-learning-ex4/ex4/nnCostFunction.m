...
% 
% for i=1:m
%     J = J + (-Y(i,:)*log(h_trans(:,i)) - (1-Y(i,:))*log(1-h_trans(:,i)));
% end
% 
% 
% for i=1:m
%     del3 = h(i,:)-Y(i,:);
%     del2 = (del3*Theta2);
%     del2 = del2(2:end).*sigmoidGradient(Z2(i,:));
...
%     reg = reg + Theta1(i,:)*Theta1_trans(:,i);
% end
% 
% for i=1:size(Theta2,1)
%     reg = reg + Theta2(i,:)*Theta2_trans(:,i);
% end
% 
% reg = reg*(lambda)/(2*m);
% 
...
% 
% Theta2_grad = [Theta2_grad(:,1)/m Theta2_grad(:,2:end)/m + (lambda/m)*t_2];
% Theta1_grad = [Theta1_grad(:,1)/m Theta2_grad(:,2:end)/m + (lambda/m)*t_1];
% 
% t_1 = sum(sum(t_1.^2));
% t_2 = sum(sum(t_2.^2));
% 
% r = lambda*(t_1 + t_2)/(2*m);
% 
% J = s/m + r;
